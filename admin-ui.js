...
function render(){
 ...
 box.innerHTML=accounts.map(u=>{const owned=u.shop?.owned||[];return `<article class="admin-account">
  <div class="admin-user" data-max-user="${esc(u.id)}" role="link" tabindex="0" aria-label="Открыть профиль в MAX"><div class="admin-avatar">${u.photo?'<img src="'+esc(u.photo)+'" alt="">':'♟️'}</div><div class="admin-user-main"><b>${esc(u.name||'Игрок')}</b><span>ID: ${esc(u.id)}</span>${u.username?'<span>@'+esc(u.username)+'</span>':''}</div>${String(u.id)==='163701646'?'<em>АДМИН</em>':''}</div>
 ...
 box.querySelectorAll('[data-max-user]').forEach(b=>{const openUser=()=>{const id=String(b.dataset.maxUser||'').trim();if(!id)return;const url='max://user/'+encodeURIComponent(id);try{if(window.WebApp?.openLink)window.WebApp.openLink(url);else window.location.href=url}catch{window.location.href=url}};b.addEventListener('click',openUser);b.addEventListener('keydown',e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();openUser()}})});
