...
async function api(body=null){
 ...
 const opts=body?{method:'POST',headers:{'content-type':'application/json','x-max-init-data':init},body:JSON.stringify(body)}:{headers:{'x-max-init-data':init}};
...
function render(){
 ...
 box.querySelectorAll('[data-board]').forEach(b=>b.onclick=async()=>{b.disabled=true;try{const own=b.classList.contains('is-owned');await api({action:own?'revoke':'grant',userId:b.dataset.user,boardId:b.dataset.board});await load();notice(own?'Доска убрана':'Доска добавлена')}catch(e){notice(e.message)}finally{b.disabled=false}});
 box.querySelectorAll('[data-reset]').forEach(b=>b.onclick=async()=>{if(!confirm('Очистить купленные доски у этого аккаунта? Оригинал останется бесплатно.'))return;b.disabled=true;try{await api({action:'reset',userId:b.dataset.reset});await load();notice('Покупки очищены')}catch(e){notice(e.message)}finally{b.disabled=false}});
}
