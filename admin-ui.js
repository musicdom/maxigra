...
function render(){
 ...
 box.innerHTML=accounts.map(u=>{const owned=u.shop?.owned||[];return `<article class="admin-account">
  <button type="button" class="admin-user admin-user-link" data-max-user="${esc(u.id)}" aria-label="Открыть профиль в MAX"><div class="admin-avatar">${u.photo?'<img src="'+esc(u.photo)+'" alt="">':'♟️'}</div><div class="admin-user-main"><b>${esc(u.name||'Игрок')}</b><span>ID: ${esc(u.id)}</span>${u.username?'<span>@'+esc(u.username)+'</span>':''}</div>${String(u.id)==='163701646'?'<em>АДМИН</em>':''}</button>
 ...
 box.querySelectorAll('[data-max-user]').forEach(b=>b.onclick=()=>{const id=String(b.dataset.maxUser||'').trim();if(!id)return;const url='https://max.ru/u/'+encodeURIComponent(id);try{if(window.WebApp?.openLink)window.WebApp.openLink(url);else window.open(url,'_blank')}catch{window.location.href=url}});
