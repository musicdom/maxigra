...
const call=(method,key,value)=>{
 const storage=getStorage();
 if(!storage?.[method])return Promise.resolve('');
 try{
  const p=value===undefined?storage[method](PREFIX+key):storage[method](PREFIX+key,value);
  const out=p&&typeof p.then==='function'?p.then(v=>String(v?.value??v??'')):Promise.resolve(String(p?.value??p??''));
  return Promise.race([out,new Promise(resolve=>setTimeout(()=>resolve(''),1500))]);
...
async function cacheOne(k){
 if(memory.has(k))return;
 try{
  const v=await fetchDataUrl(ASSETS[k]);
  if(v){
   memory.set(k,v);
   applyCss();
   // Сохранение не должно задерживать показ меню.
   void call('setItem',k,v);
   void idbSet(k,v);
  }
 }catch{}
}
