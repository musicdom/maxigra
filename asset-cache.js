...
const call=(method,key,value)=>{if(!storage?.[method])return Promise.resolve('');try{const p=value===undefined?storage[method](PREFIX+key):storage[method](PREFIX+key,value);return p&&typeof p.then==='function'?p.then(v=>String(v?.value??v??'')):Promise.resolve(String(p?.value??p??''));}catch{return Promise.resolve('')}};
