...
async function saveInventory(id,state){
  await redis('SET',[INVENTORY_KEY(id),JSON.stringify({owned:state.owned,selectedBoard:state.selectedBoard,selectedPieces:state.selectedPieces||'default',ai:Number(state.ai)||1,hints:Number(state.hints)||0})]);
}
...
export async function POST(request){
  try{
    ...
    if(action==='reset'){state.owned=['board_original'];state.selectedBoard='board_original';}
