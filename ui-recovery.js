...
function boot(){
  ...
  $('online-hard-cancel').onclick=async()=>{hide();if(window.CheckersOnline?.cancel)await window.CheckersOnline.cancel();else show('menu-screen')};
  window.addEventListener('online-search-message',e=>{const text=$('online-hard-text');if(text&&e.detail)text.textContent=e.detail});window.addEventListener('online-game-started',hide);
  setInterval(()=>{
    const gameScreen=$('game-screen'),result=$('game-result');
    if(gameScreen?.classList.contains('active')||result?.classList.contains('show')||window.CheckersOnline?.active===false)hide();
  },200);
}
