...
const state=local;
state.owned=[];state.selectedBoard='default';state.selectedPieces='default';state.ai=Math.max(1,Math.min(4,Number(state.ai)||1));state.hints=Math.max(0,Number(state.hints)||0);state.games=Number(state.games)||0;state.wins=Number(state.wins)||0;state.losses=Number(state.losses)||0;state.draws=Number(state.draws)||0;state.coins=0;
const save=()=>{try{localStorage.setItem(KEY,JSON.stringify({...state,owned:[],coins:0}))}catch(e){}};
let readyPromise=null;
async function sync(){
 if(readyPromise)return readyPromise;
 readyPromise=(async()=>{
  const init=window.WebApp?.initData||''; if(!init)throw new Error('MAX_INIT_DATA_REQUIRED');
  const r=await fetch('/api/shop',{headers:{'x-max-init-data':init},cache:'no-store'});
...
async function buy(id){
 if(!catalog[id]||owned(id))return false;
 const init=window.WebApp?.initData||'';if(!init)throw new Error('MAX_INIT_DATA_REQUIRED');
 const r=await fetch('/api/shop',{method:'POST',headers:{'content-type':'application/json','x-max-init-data':init},body:JSON.stringify({action:'purchase',id}),cache:'no-store'});
 const d=await r.json().catch(()=>({ok:false,error:'BAD_RESPONSE'}));
 if(!r.ok||!d.ok)throw new Error(d.error||`HTTP_${r.status}`);
 Object.assign(state,d.state||{});save();return true;
}
async function selectBoard(id){if(!owned(id))return false;await select(id);return true}
async function selectPieces(id){if(!owned(id))return false;await select(id);return true}
async function select(id){
 const init=window.WebApp?.initData||'';if(!init)throw new Error('MAX_INIT_DATA_REQUIRED');
 const r=await fetch('/api/shop',{method:'POST',headers:{'content-type':'application/json','x-max-init-data':init},body:JSON.stringify({action:'select',id}),cache:'no-store'});
 const d=await r.json().catch(()=>({ok:false,error:'BAD_RESPONSE'}));
 if(!r.ok||!d.ok)throw new Error(d.error||`HTTP_${r.status}`);
 Object.assign(state,d.state||{});save();return true;
}
