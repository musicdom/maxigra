...
function allMoves(side,b){const force=hasCapture(side,b),out=[];for(const from of pieces(side,b)){if(force){const walk=(pos,state,path)=>{const ms=captures(pos.r,pos.c,state);if(!ms.length){if(path.length)out.push({from:{...from},move:path[0],sequence:path,board:state});return}for(const m of ms)walk({r:m.r,c:m.c},applyMove(state,pos,m),path.concat(m))};walk(from,b,[])}else for(const m of simpleMoves(from.r,from.c,b))out.push({from:{...from},move:m,sequence:[m],board:applyMove(b,from,m)})}return out}
